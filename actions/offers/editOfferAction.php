...
if(isset($_POST['publish'])){
    if(!empty($_POST['titleOffer']) AND !empty($_POST['descriptionOffer']) AND !empty($_POST['locOffer']) AND !empty($_POST['salaryOffer'])){
        ...
        $editOffer = $bdd->prepare('UPDATE offers SET title = ?, description = ?, location = ?, salary = ? WHERE id = ?');
        $editOffer->execute(array($new_title_offer, $new_description_offer, $new_loc_offer, $new_salary_offer, $offerId));
